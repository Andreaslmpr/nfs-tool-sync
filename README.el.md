...
> 🌐 Η τεκμηρίωση είναι διαθέσιμη στα **Ελληνικά** και στα **Αγγλικά**.
...
σε εφεδρικό κέντρο δεδομένων, με ένα `add` ανά κύκλο από scheduler (cron, systemd timer).
Ο μετρητής σφαλμάτων ανά ζεύγος και τα `[ERROR]` στο log τροφοδοτούν alerting για κόμβο που έμεινε πίσω.
